    ...
    function(insertSizeHistogram) {
        var run = function() {
            QUnit.test('insertSizeHistogram tests', function() {
				...
				var hist;
                var w = jQuery(div).data("width");
                var h = jQuery(div).data("height");
                hist=insertSizeHistogram.drawChart({'data': aJSON, 'width': w, 'height': h}); 
                jQuery(div).append( function(){return hist.svg.node();} );
