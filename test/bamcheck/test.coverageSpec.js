define(['chai', '../js/src/coverage.js', '../js/src/readBCfile.js'], function (chai, coverage, read) {
	var assert = chai.assert;
	var should = chai.should();
	var expect = chai.expect;
	describe('Coverage distribution graph tests', function () {
		it('should create a new graph', function () {
			expect(coverage(read("demo/sample_1.bc"))).to.be.an('object');
		});
		it('should not create a new graph and return null', function () {
			expect(coverage(read('     fgafg   '))).to.be.null;
		});
		it('should not create a new graph and return null', function () {
