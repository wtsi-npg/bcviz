...
function newAdapterChart (data, divID, title, width, height) {
    ...
    forwardData.yMax = d3.max(forwardData.formattedData, function (d) { return d.yVar; });
    reverseData.yMax = d3.max(reverseData.formattedData, function (d) { return d.yVar; });
...
function format_adapter_chart (data) {
    ...
    for(var k in data.start_counts){
        formattedData.push({xVar: parseInt(k), yVar: data.start_counts[k]});
    }
