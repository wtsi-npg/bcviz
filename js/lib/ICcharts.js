define(['lib/dotPlot'], function(dotPlot){
    var keysIC = {
        ...
        if(data && data[0] && data[0][1] && data[0][1].values && data[0][1].values.length !== 0){
            if(split){
                ...
                if(legend){
                    ...
                    returnValue = [new dotPlot(data[0], divID, false, "Indels per cycle(forward)", keysIC.fwd, width, height), new dotPlot(data[0], divID, false, "Indels per cycle(reverse)", keysIC.reverse, width, height)];
                }
